...
static inline uint_fast32_t mod_mul(uint_fast32_t a, uint_fast32_t b) {
  uint_fast32_t temp;  //uint_fast64_t temp; 
  uint_fast32_t temp2;
  assert (a <= 0x7FFFFFFE);
  assert (b <= 0x7FFFFFFE);
#if 0
...
#else
  temp = (uint_fast32_t)a * b;     // temp = (uint_fast64_t)a * b;     /* temp <= 0x3FFFFFFE00000004 */  
...
#else
  temp = (uint_fast32_t)a * b + sum; //temp = (uint_fast64_t)a * b + sum;   /* temp <= 0x3FFFFFFE80000002 */
