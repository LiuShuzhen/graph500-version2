...
#ifdef FAST_64BIT_ARITHMETIC  //#ifdef FAST_64BIT_ARITHMETIC
...
*/
  x = (x >> 32) | (x << 32);
  x = ((x >> 16) & UINT32_C(0x0000FFFF0000FFFF)) | ((x & UINT32_C(0x0000FFFF0000FFFF)) << 16);
  x = ((x >>  8) & UINT32_C(0x00FF00FF00FF00FF)) | ((x & UINT32_C(0x00FF00FF00FF00FF)) <<  8);
#endif
  x = ((x >>  4) & UINT32_C(0x0F0F0F0F0F0F0F0F)) | ((x & UINT32_C(0x0F0F0F0F0F0F0F0F)) <<  4);
  x = ((x >>  2) & UINT32_C(0x3333333333333333)) | ((x & UINT32_C(0x3333333333333333)) <<  2);
  x = ((x >>  1) & UINT32_C(0x5555555555555555)) | ((x & UINT32_C(0x5555555555555555)) <<  1);
  return x;

#else

  /* 32-bit code */
  uint32_t h = (uint32_t)(x >> 32);// uint32_t h = (uint32_t)(x >> 32);
  ...
  return ((uint32_t)l << 16) | h;  // return ((uint64_t)l << 32) | h;  /* Swap halves */

#endif
}

/* Apply a permutatio to scramble vertex numbers; a randomly generated
...
static inline int32_t scramble(int32_t v0, int lgN, uint64_t val0, uint64_t val1) {
  uint32_t v = (uint32_t)v0;  //uint64_t v = (uint64_t)v0; 
  v += val0 + val1;
  v *= (val0 | UINT32_C(0x4519840211493211));  //v *= (val0 | UINT64_C(0x4519840211493211)); 
  v = (bitreverse(v) >> (64 - lgN));
  assert ((v >> lgN) == 0);
  v *= (val1 | UINT32_C(0x3050852102C843A5));  //v *= (val1 | UINT64_C(0x3050852102C843A5));
  v = (bitreverse(v) >> (64 - lgN));
  assert ((v >> lgN) == 0);
  return (int32_t)v;   //return (int64_t)v; 
}
...
void make_one_edge(int32_t nverts, int level, int lgN, mrg_state* st, packed_edge* result, uint64_t val0, uint64_t val1) {
  int32_t base_src = 0, base_tgt = 0;   //int64_t base_src = 0, base_tgt = 0;  
  while (nverts > 1) {
...
void generate_kronecker_range(
       ...
       int logN /* In base 2 */,
       int32_t start_edge, int32_t end_edge,  //int64_t start_edge, int64_t end_edge, 
       packed_edge* edges) {
  mrg_state state;
  int32_t nverts = (int32_t)1 << logN; //int64_t nverts = (int64_t)1 << logN; 
  int32_t ei;  //  int64_t ei; 

  mrg_seed(&state, seed);

  uint64_t val0, val1; // uint64_t val0, val1;                /* Values for scrambling */
  {
    mrg_state new_state = state;
    mrg_skip(&new_state, 50, 7, 0);
    val0 = mrg_get_uint_orig(&new_state);
    val0 *= UINT32_C(0xFFFFFFFF);   //val0 *= UINT64_C(0xFFFFFFFF); 
    val0 += mrg_get_uint_orig(&new_state);
    val1 = mrg_get_uint_orig(&new_state);
    val1 *= UINT32_C(0xFFFFFFFF);   //val1 *= UINT64_C(0xFFFFFFFF); 
    val1 += mrg_get_uint_orig(&new_state);
  }
...
#pragma mta block schedule
  ...
  for (ei = start_edge; ei < end_edge; ++ei) {
    mrg_state new_state = state;
    mrg_skip(&new_state, 0, (uint32_t)ei, 0);             
