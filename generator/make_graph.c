...
static void compute_edge_range(int rank, int size, int32_t M, int32_t* start_idx, int32_t* end_idx)   {
  int32_t rankc = (int32_t)(rank);//int64_t rankc = (int64_t)(rank);
  int32_t sizec = (int32_t)(size);//int64_t sizec = (int64_t)(size);
...
void make_graph(int log_numverts, int32_t M, uint32_t userseed1, uint32_t userseed2, int32_t* nedges_ptr_in, packed_edge** result_ptr_in) {
  /* Add restrict to input pointers. */
  int32_t* restrict nedges_ptr = nedges_ptr_in;// int64_t* restrict nedges_ptr = nedges_ptr_in;
...
void make_graph(int log_numverts, int32_t M, uint32_t userseed1, uint32_t userseed2, int32_t* nedges_ptr, packed_edge** result_ptr) {
  int rank, size;
  ...
  int32_t start_idx, end_idx;  //int64_t start_idx, end_idx; 
  compute_edge_range(rank, size, M, &start_idx, &end_idx);
  int32_t nedges = end_idx - start_idx;  //int64_t nedges = end_idx - start_idx; 
...
void make_random_numbers(
       /* in */ int32_t nvalues ,   // int64_t nvalues   /* Number of values to generate */,
       /* in */ uint32_t userseed1,  // uint64_t userseed1, /* Arbitrary 64-bit seed value */,
       /* in */ uint32_t userseed2,  //uint64_t userseed2,  /* Arbitrary 64-bit seed value */,
       /* in */ int32_t position,  // int64_t position,  /* Start index in random number stream */,
       /* out */ double* result    /* Returned array of values */
) {
  int32_t i;  //int64_t i;
  uint_fast32_t seed[5];//uint_fast32_t seed[5];
  make_mrg_seed(userseed1, userseed2, seed);

  mrg_state st;
  mrg_seed(&st, seed);
//mrg_skip(&st, 2, 0, 2 * (uint64_t)position);
  mrg_skip(&st, 2, 0, 2 * (uint32_t)position); /* Each double takes two PRNG outputs */
