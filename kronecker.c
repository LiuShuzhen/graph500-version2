...
kronecker_edgelist (struct packed_edge *IJ_in, int64_t nedge, int64_t SCALE,
		    double A, double B, double C)  //line15,16
*/
kronecker_edgelist (struct packed_edge *IJ_in, int32_t nedge, int32_t SCALE,
		    double A, double B, double C)  //line15,16
{
  const int32_t nvtx = 1L<<SCALE;    //  const int64_t nvtx = 1L<<SCALE; 
  struct packed_edge * restrict IJ = IJ_in;
  int32_t * restrict vperm = NULL;    // int64_t * restrict vperm = NULL; 
  double D;
...
#else /* __MTA__ */
  int32_t rank, size;       //int64_t rank, size;    //line39
