...
uint32_t userseed;           //uint64_t userseed; 
 ...
 * range. */
static void make_mrg_seed(uint32_t userseed, uint_fast32_t* seed)  // static void make_mrg_seed(uint64_t userseed, uint_fast32_t* seed)
{
  seed[0] = (userseed & 0x3FFFFFFF) + 1;
  seed[1] = ((userseed >> 30) & 0x3FFFFFFF) + 1;
  seed[2] = (userseed & 0x3FFFFFFF) + 1;
  seed[3] = ((userseed >> 30) & 0x3FFFFFFF) + 1;
  seed[4] = ((userseed >> 30) << 4) + (userseed >> 30) + 1; //seed[4] = ((userseed >> 60) << 4) + (userseed >> 60) + 1;
}
