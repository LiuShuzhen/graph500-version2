		...
		int64_t nv, const int64_t * restrict bfs_tree, int64_t root)
{
  ...
  OMP("omp parallel shared(err)") {
	...
	if (!terr && bfs_tree[k] >= 0 && k != root) {
	  int64_t parent = k;//int64_t parent = k;
	  int64_t nhop = 0;//int64_t nhop = 0;
	  ...
	  if (!terr) {
	    ...
	    nhop = 0;
	    parent = k;
	    int32_t lastlvl = level[k]+1;  // int64_t lastlvl = level[k]+1; 
	    while (level[parent] > 0) {
...
*/
int32_t 
verify_bfs_tree (int32_t *bfs_tree_in, int32_t max_bfsvtx,
		 int32_t root,
		 const struct packed_edge *IJ_in, int32_t nedge)
{
  int64_t * restrict bfs_tree = bfs_tree_in;  // int64_t * restrict bfs_tree = bfs_tree_in; (line 90)
  const struct packed_edge * restrict IJ = IJ_in;

  int err;
  int64_t nedge_traversed;  // int64_t nedge_traversed;
  int64_t * restrict seen_edge, * restrict level;  // int64_t * restrict seen_edge, * restrict level; 

  const int64_t nv = max_bfsvtx+1;  //const int64_t nv = max_bfsvtx+1; 
  ...
  */
  ...
  OMP("omp parallel shared(err)") {
    ...
    MTA("mta assert parallel") MTA("mta use 100 streams")
      for (k = 0; k < nedge; ++k) {
	const int64_t i = get_v0_from_edge (&IJ[k]);  //const int64_t i = get_v0_from_edge (&IJ[k]);
	const int64_t j = get_v1_from_edge (&IJ[k]);   //const int64_t j = get_v1_from_edge (&IJ[k]); 
	int64_t lvldiff;  //int64_t lvldiff; 
	terr = err;
