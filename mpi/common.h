...
#define VERTEX_TO_GLOBAL(r, i) ((int32_t)(MUL_SIZE((uint32_t)((i))) + (int)((r))))
#else
...
#define VERTEX_TO_GLOBAL(r, i) ((int32_t)(((MUL_SIZE((uint32_t)((i) / ulong_bits_squared)) + (r)) * ulong_bits_squared) + ((i) % ulong_bits_squared)))
#endif
...
typedef struct tuple_graph {
  ...
  int32_t edgememory_size;  //int64_t edgememory_size; 
  int32_t max_edgememory_size;  //int64_t max_edgememory_size; 
  MPI_File edgefile; /* Or MPI_FILE_NULL if edges are in memory */
  int32_t nglobaledges; //   int64_t nglobaledges;  /* Number of edges in graph, in both cases */
} tuple_graph;
...
/*int64_t edge_count_i = (int64_t)(-1);*/ \
    int32_t edge_count_i = (int32_t)(-1); \
...
/*start_edge_index = int64_min(FILE_CHUNKSIZE * block_idx, (tg)->edgememory_size);*/ \
        start_edge_index = int32_min(FILE_CHUNKSIZE * block_idx, (tg)->edgememory_size); \
/*end_edge_index = int64_min(start_edge_index + FILE_CHUNKSIZE, (tg)->degememory_size)*/; \
        end_edge_index = int32_min(start_edge_index + FILE_CHUNKSIZE, (tg)->edgememory_size); \
      } \
...
static inline int32_t tuple_graph_max_bufsize(const tuple_graph* tg) {
...
int lg_int32_t(int32_t x);// int lg_int64_t(int64_t x);  /* In utils.c */
...
*/
int validate_bfs_result(const tuple_graph* const tg, const int32_t nglobalverts, const size_t nlocalverts, const int32_t root, int32_t* const pred, int32_t* const edge_visit_count_ptr); /* In validate.c */
...
void run_bfs(int32_t root, int32_t* pred);//void run_bfs(int64_t root, int64_t* pred);
//void get_vertex_distribution_for_pred(size_t count, const int64_t* vertices, int* owners, size_t* locals);
void get_vertex_distribution_for_pred(size_t count, const int32_t* vertices, int* owners, size_t* locals);
//int64_t vertex_to_global_for_pred(int v_rank, size_t v_local); /* Only used for error messages */
int32_t vertex_to_global_for_pred(int v_rank, size_t v_local); /* Only used for error messages */
