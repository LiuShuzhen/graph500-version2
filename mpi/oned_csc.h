...
typedef struct oned_csc_graph {
  size_t nlocalverts;
  int32_t max_nlocalverts;//int64_t max_nlocalverts;
  size_t nlocaledges;
  int lg_nglobalverts;
  int32_t nglobalverts;// int64_t nglobalverts;
  size_t *rowstarts;
  int32_t *column;//  int64_t *column;
...
#define UNSWIZZLE_VERTEX(c) (MUL_SIZE((c) & ((INT32_C(1) << lg_local_queue_size) * ULONG_BITS - 1)) | ((((c) / ULONG_BITS) >> lg_local_queue_size)))
