...
void RF_FUNCNAME(const tuple_graph* const tg, RF_EXTRA_PARAMS) { \
  ...
  uint32_t max_vertex = UINT32_C(0); /* uint64_t max_vertex = UINT64_C(0);*/ \
  ...
  do { \
    ...
    ITERATE_TUPLE_GRAPH_BEGIN(tg, buf, bufsize) { \
      ptrdiff_t i; \
      uint32_t prev_max_vertex = max_vertex;  /*uint64_t prev_max_vertex = max_vertex; */ \
      ...
      for (i = 0; i < bufsize; ++i) { \
        ...
        if (v0 == v1) continue; \
        max_vertex |= (uint32_t)(v0 | v1); /* max_vertex |= (uint64_t)(v0 | v1);*/ \
      } \
      MPI_Allreduce(MPI_IN_PLACE, &max_vertex, 1, MPI_UINT32_T, MPI_BOR, MPI_COMM_WORLD); /* MPI_Allreduce(MPI_IN_PLACE, &max_vertex, 1, MPI_UINT64_T, MPI_BOR, MPI_COMM_WORLD);*/\
    ...
    /*fprintf(stderr, "Restarting because of change of max_vertex from %" PRIu64 " to %" PRIu64 "\n", prev_max_vertex, max_vertex); */ \
          fprintf(stderr, "Restarting because of change of max_vertex from %" PRIu32 " to %" PRIu32 "\n", prev_max_vertex, max_vertex); \
        } \
        restart = 1; \
