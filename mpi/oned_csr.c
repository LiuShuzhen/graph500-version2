...
static void make_csr(const packed_edge* restrict const inbuf, temp_csr_graph* restrict const outg /* Must have memory and nlocalverts/nlocaledges filled in */) {
  ...
  int64_t* restrict column = outg->column;//int64_t* restrict column = outg->column;
  {
...
#pragma omp parallel for
    for (i = 0; i < (ptrdiff_t)inbuf_size; ++i) {
      ...
      int32_t v1 = get_v1_from_edge(&inbuf[i]);// int64_t v1 = get_v1_from_edge(&inbuf[i]);
...
//b->column = (int64_t*)xrealloc(b->column, new_alloc * sizeof(int64_t));
    b->column = (int32_t*)xrealloc(b->column, new_alloc * sizeof(int32_t));
  }
...
// (b_nlocaledges - b->rowstarts[a_nlocalverts]) * sizeof(int64_t));
          (b_nlocaledges - b->rowstarts[a_nlocalverts]) * sizeof(int32_t));
  ptrdiff_t i_plus_1;
...
/*size_t nlocalverts_so_far = (size_t)DIV_SIZE((UINT64_C(1) << (LG_NGLOBALVERTS_SO_FAR)) / ulong_bits_squared + size - 1) * ulong_bits_squared;*/ \
  size_t nlocalverts_so_far = (size_t)DIV_SIZE((UINT32_C(1) << (LG_NGLOBALVERTS_SO_FAR)) / ulong_bits_squared + size - 1) * ulong_bits_squared; \
  temp_csr_graph t = { \
...
/*g->column = (int64_t*)xrealloc(graph_so_far.column, (size_t)g->nlocaledges * sizeof(int64_t));*/ \
  ...
  int32_t nlocalverts = (int32_t)(graph_so_far.nlocalverts); /* int64_t nlocalverts = (int64_t)(graph_so_far.nlocalverts);*/\
  g->nlocalverts = (size_t)nlocalverts; \
/*MPI_Allreduce(&nlocalverts, &g->max_nlocalverts, 1, MPI_INT64_T, MPI_MAX, MPI_COMM_WORLD);*/ \
  MPI_Allreduce(&nlocalverts, &g->max_nlocalverts, 1, MPI_INT32_T, MPI_MAX, MPI_COMM_WORLD); \
  g->lg_nglobalverts = graph_so_far.lg_nglobalverts; \
  g->nglobalverts = INT32_C(1) << graph_so_far.lg_nglobalverts; // g->nglobalverts = INT64_C(1) << graph_so_far.lg_nglobalverts;
