...
static inline int32_t get_pred_from_pred_entry(int32_t val) {
  return (val << 16) >> 16;
}
//static inline uint16_t get_depth_from_pred_entry(int64_t val)
static inline uint16_t get_depth_from_pred_entry(int32_t val) {
...
*/
static inline void write_pred_entry_depth(int32_t* loc, uint16_t depth) {
  *loc = (*loc & INT32_C(0xFFFFFF)) | ((int32_t)(depth & 0xFFFF) << 48);
}
...
static int check_value_ranges(const int32_t nglobalverts, const size_t nlocalverts, const int32_t* const pred) {
...
#pragma omp parallel for reduction(||:any_range_errors)
      for (i = i_start; i < i_end; ++i) {
        int32_t p = get_pred_from_pred_entry(pred[i]);//int64_t p = get_pred_from_pred_entry(pred[i]);
...
//fprintf(stderr, "%d: Validation error: parent of vertex %" PRId64 " is out-of-range value %" PRId64 ".\n", rank, vertex_to_global_for_pred(rank, i), p);
          fprintf(stderr, "%d: Validation error: parent of vertex %" PRId32 " is out-of-range value %" PRId32 ".\n", rank, vertex_to_global_for_pred(rank, i), p);
          any_range_errors = 1;
        }
      }
    }
...
static int build_bfs_depth_map(const int32_t nglobalverts, const size_t nlocalverts, const size_t maxlocalverts, const int32_t root, int32_t* const pred) {
  (void)nglobalverts;
...
//int64_t* restrict pred_pred = (int64_t*)xMPI_Alloc_mem(size_min(CHUNKSIZE, nlocalverts) * sizeof(int64_t)); 
  int32_t* restrict pred_pred = (int32_t*)xMPI_Alloc_mem(size_min(CHUNKSIZE, nlocalverts) * sizeof(int32_t)); /* Predecessor info of predecessor vertex for each local vertex */
// gather* pred_win = init_gather((void*)pred, nlocalverts, sizeof(int64_t), pred_pred, size_min(CHUNKSIZE, nlocalverts), size_min(CHUNKSIZE, nlocalverts), MPI_INT64_T);
  gather* pred_win = init_gather((void*)pred, nlocalverts, sizeof(int32_t), pred_pred, size_min(CHUNKSIZE, nlocalverts), size_min(CHUNKSIZE, nlocalverts), MPI_INT32_T);
//  int64_t* restrict pred_vtx = (int64_t*)xmalloc(size_min(CHUNKSIZE, nlocalverts) * sizeof(int64_t)); 
  int32_t* restrict pred_vtx = (int32_t*)xmalloc(size_min(CHUNKSIZE, nlocalverts) * sizeof(int32_t)); /* Vertex (not depth) part of pred map */
...
fprintf(stderr, "%d: Validation error: BFS predecessors do not form a tree; see vertices %" PRId64 " (depth %" PRIu16 ") and %" PRId64 " (depth %" PRIu16 ").\n", rank, vertex_to_global_for_pred(rank, i), get_depth_from_pred_entry(pred[i]), get_pred_from_pred_entry(pred[i]), get_depth_from_pred_entry(pred_pred[i - i_start]));
              validation_passed = 0;
*/
              fprintf(stderr, "%d: Validation error: BFS predecessors do not form a tree; see vertices %" PRId32 " (depth %" PRIu16 ") and %" PRId32 " (depth %" PRIu16 ").\n", rank, vertex_to_global_for_pred(rank, i), get_depth_from_pred_entry(pred[i]), get_pred_from_pred_entry(pred[i]), get_depth_from_pred_entry(pred_pred[i - i_start]));
              validation_passed = 0;
...
*/
static int check_bfs_depth_map_using_predecessors(const tuple_graph* const tg, const int32_t nglobalverts, const size_t nlocalverts, const size_t maxlocalverts, const int32_t root, const int32_t* const pred) {
...
// fprintf(stderr, "%d: Validation error: depth of root vertex %" PRId64 " is %" PRIu16 ", not 0.\n", rank, root, get_depth_from_pred_entry(pred[root_local]));
      fprintf(stderr, "%d: Validation error: depth of root vertex %" PRId32 " is %" PRIu16 ", not 0.\n", rank, root, get_depth_from_pred_entry(pred[root_local]));
      validation_passed = 0;
    }
...
*/
        fprintf(stderr, "%d: Validation error: depth of vertex %" PRId32 " with no predecessor is %" PRIu16 ", not UINT16_MAX.\n", rank, vertex_to_global_for_pred(rank, i), get_depth_from_pred_entry(pred[i]));
        validation_passed = 0;
...
*/
        fprintf(stderr, "%d: Validation error: predecessor of claimed unreachable vertex %" PRId32 " is %" PRId32 ", not -1.\n", rank, vertex_to_global_for_pred(rank, i), get_pred_from_pred_entry(pred[i]));
        validation_passed = 0;
      }
    }
  }
//int64_t* restrict pred_pred = (int64_t*)xMPI_Alloc_mem(size_min(CHUNKSIZE, nlocalverts) * sizeof(int64_t));
  int64_t* restrict pred_pred = (int32_t*)xMPI_Alloc_mem(size_min(CHUNKSIZE, nlocalverts) * sizeof(int32_t)); /* Predecessor info of predecessor vertex for each local vertex */
// gather* pred_win = init_gather((void*)pred, nlocalverts, sizeof(int64_t), pred_pred, size_min(CHUNKSIZE, nlocalverts), size_min(CHUNKSIZE, nlocalverts), MPI_INT64_T);
  gather* pred_win = init_gather((void*)pred, nlocalverts, sizeof(int32_t), pred_pred, size_min(CHUNKSIZE, nlocalverts), size_min(CHUNKSIZE, nlocalverts), MPI_INT32_T);
//int64_t* restrict pred_vtx = (int64_t*)xmalloc(size_min(CHUNKSIZE, nlocalverts) * sizeof(int64_t));
  int64_t* restrict pred_vtx = (int32_t*)xmalloc(size_min(CHUNKSIZE, nlocalverts) * sizeof(int32_t)); /* Vertex (not depth) part of pred map */
  int* restrict pred_owner = (int*)xmalloc(size_min(CHUNKSIZE, nlocalverts) * sizeof(int));
  size_t* restrict pred_local = (size_t*)xmalloc(size_min(CHUNKSIZE, nlocalverts) * sizeof(size_t));
  size_t ii;
...
*/
        fprintf(stderr, "%d: Validation error: predecessor %" PRId32 " of vertex %" PRId32 " (depth %" PRIu16 ") is marked as unreachable.\n", rank, get_pred_from_pred_entry(pred[i]), vertex_to_global_for_pred(rank, i), get_depth_from_pred_entry(pred[i]));
        validation_passed = 0;
      }
...
// fprintf(stderr, "%d: Validation error: root vertex %" PRId64 " is invalid.\n", rank, root);
    fprintf(stderr, "%d: Validation error: root vertex %" PRId32 " is invalid.\n", rank, root);
    ranges_ok = 0;
  }
  ...
  uint32_t maxlocalverts_ui = nlocalverts;//uint64_t maxlocalverts_ui = nlocalverts;
// MPI_Allreduce(MPI_IN_PLACE, &maxlocalverts_ui, 1, MPI_UINT64_T, MPI_MAX, MPI_COMM_WORLD);
  MPI_Allreduce(MPI_IN_PLACE, &maxlocalverts_ui, 1, MPI_UINT32_T, MPI_MAX, MPI_COMM_WORLD);
...
*/
      fprintf(stderr, "%d: Validation error: parent of root vertex %" PRId32 " is %" PRId32 ", not the root itself.\n", rank, root, get_pred_from_pred_entry(pred[root_local]));
      validation_passed = 0;
    }
  }
...
// fprintf(stderr, "%d: Validation error: parent of non-root vertex %" PRId64 " is itself.\n", rank, vertex_to_global_for_pred(rank, i));
          fprintf(stderr, "%d: Validation error: parent of non-root vertex %" PRId32 " is itself.\n", rank, vertex_to_global_for_pred(rank, i));
          validation_passed = 0;
        }
      }
    }
...
//int64_t* restrict edge_endpoint = (int64_t*)xmalloc(2 * edge_chunk_size * sizeof(int64_t));
    int32_t* restrict edge_endpoint = (int32_t*)xmalloc(2 * edge_chunk_size * sizeof(int32_t));
...
//int64_t* restrict edge_preds = (int64_t*)xMPI_Alloc_mem(2 * edge_chunk_size * sizeof(int64_t));
    int32_t* restrict edge_preds = (int32_t*)xMPI_Alloc_mem(2 * edge_chunk_size * sizeof(int32_t));
//gather* pred_win = init_gather((void*)pred, nlocalverts, sizeof(int64_t), edge_preds, 2 * edge_chunk_size, 2 * edge_chunk_size, MPI_INT64_T);
    gather* pred_win = init_gather((void*)pred, nlocalverts, sizeof(int32_t), edge_preds, 2 * edge_chunk_size, 2 * edge_chunk_size, MPI_INT32_T);
    unsigned char one = 1;
    ...
    int32_t edge_visit_count = 0;//   int64_t edge_visit_count = 0;
...
#pragma omp parallel for
        for (i = i_start; i < i_end; ++i) {
          int32_t v0 = get_v0_from_edge(&buf[i]);//  int64_t v0 = get_v0_from_edge(&buf[i]);
          int32_t v1 = get_v1_from_edge(&buf[i]);//int64_t v1 = get_v1_from_edge(&buf[i]);
...
#pragma omp parallel for reduction(&&:validation_passed) reduction(+:edge_visit_count)
        for (i = i_start; i < i_end; ++i) {
          int32_t src = get_v0_from_edge(&buf[i]);// int64_t src = get_v0_from_edge(&buf[i]);
          int32_t tgt = get_v1_from_edge(&buf[i]);//int64_t tgt = get_v1_from_edge(&buf[i]);
...
*/
            fprintf(stderr, "%d: Validation error: edge connects vertex %" PRId32 " in the BFS tree (depth %" PRIu16 ") to vertex %" PRId32 " outside the tree.\n", rank, src, src_depth, tgt);
            validation_passed = 0;
...
*/
            fprintf(stderr, "%d: Validation error: depths of edge endpoints %" PRId32 " (depth %" PRIu16 ") and %" PRId32 " (depth %" PRIu16 ") are too far apart (abs. val. > 1).\n", rank, src, src_depth, tgt, tgt_depth);
            validation_passed = 0;
...
#pragma omp parallel for reduction(&&:validation_passed)
    for (i = 0; i < (ptrdiff_t)nlocalverts; ++i) {
      ...
      if (!found_pred_edge) {
        int32_t v = vertex_to_global_for_pred(rank, i);// int64_t v = vertex_to_global_for_pred(rank, i);
//fprintf(stderr, "%d: Validation error: no graph edge from vertex %" PRId64 " to its parent %" PRId64 ".\n", rank, v, get_pred_from_pred_entry(pred[i]));
        fprintf(stderr, "%d: Validation error: no graph edge from vertex %" PRId32 " to its parent %" PRId32 ".\n", rank, v, get_pred_from_pred_entry(pred[i]));
        validation_passed = 0;
      }
    }
    MPI_Free_mem(pred_valid);
//  MPI_Allreduce(MPI_IN_PLACE, &edge_visit_count, 1, MPI_INT64_T, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, &edge_visit_count, 1, MPI_INT32_T, MPI_SUM, MPI_COMM_WORLD);
