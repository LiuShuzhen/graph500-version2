...
typedef struct temp_csc_graph {
  ...
  int32_t* restrict column;// int64_t* restrict column;
  size_t nlocalverts;
  int lg_nglobalverts;
  int32_t nglobalverts; //int64_t nglobalverts;
  size_t nlocaledges;
...
static void make_csc(const packed_edge* restrict const inbuf, temp_csc_graph* restrict const outg /* Must have memory and nlocalverts/nglobalverts/nlocaledges filled in */) {
  ...
  int32_t* restrict column = outg->column;  // int64_t* restrict column = outg->column; 
...
#pragma omp parallel for
    for (i = 0; i < (ptrdiff_t)inbuf_size; ++i) {
      int32_t v0 = get_v0_from_edge(&inbuf[i]);// int64_t v0 = get_v0_from_edge(&inbuf[i]);
      int32_t v1 = SWIZZLE_VERTEX(get_v1_from_edge(&inbuf[i]));  // int64_t v1 = SWIZZLE_VERTEX(get_v1_from_edge(&inbuf[i]));
  ...
  if (b->lg_local_queue_size == -1) { // b is empty
    ...
    *b = *a;
    ...
    if (b_nlocaledges + a_nlocaledges > b->nlocaledges_allocated) {
      ...
      b->column = (int32_t*)xrealloc(b->column, new_alloc * sizeof(int32_t));  // b->column = (int64_t*)xrealloc(b->column, new_alloc * sizeof(int64_t));
    }
    ptrdiff_t i_plus_1;
    ...
    for (i_plus_1 = nrows; i_plus_1 > 0; --i_plus_1) {
      ...
      memmove(&b->column[b->rowstarts[i] + a->rowstarts[i]],
              ...
              (b->rowstarts[i + 1] - b->rowstarts[i]) * sizeof(int32_t));  // (b->rowstarts[i + 1] - b->rowstarts[i]) * sizeof(int64_t)); 
    }
...
#pragma omp parallel for
    for (i_plus_1 = nrows; i_plus_1 > 0; --i_plus_1) {
      ...
      memcpy(&b->column[b->rowstarts[i + 1] + a->rowstarts[i]],
             ...
             (a->rowstarts[i + 1] - a->rowstarts[i]) * sizeof(int32_t));   //  (a->rowstarts[i + 1] - a->rowstarts[i]) * sizeof(int64_t));  
    }
...
/*(int64_t*)xmalloc((size_t)(EDGES_RECEIVED_THIS_BLOCK) * sizeof(int64_t)), */ \
    /* column      */           (int32_t*)xmalloc((size_t)(EDGES_RECEIVED_THIS_BLOCK) * sizeof(int32_t)), \
    /* nlocalverts */           (size_t)(nlocalverts_so_far), \
    /* lg_nglobalverts */       (int)(LG_NGLOBALVERTS_SO_FAR), \
/*(int64_t)(INT64_C(1) << (LG_NGLOBALVERTS_SO_FAR)),*/ \
    /* nglobalverts */          (int32_t)(INT32_C(1) << (LG_NGLOBALVERTS_SO_FAR)), \
  ...
  { \
    t.lg_local_queue_size = lg_int32_t(DIV_SIZE(t_nrows)); /*t.lg_local_queue_size = lg_int64_t(DIV_SIZE(t_nrows));*/\
...
/*  graph_so_far.column = (int64_t*)xrealloc(graph_so_far.column, new_alloc_real * sizeof(int64_t));*/\
    graph_so_far.column = (int32_t*)xrealloc(graph_so_far.column, new_alloc_real * sizeof(int32_t)); \
  } \
...
/*g->column = (int64_t*)xrealloc(graph_so_far.column, (size_t)g->nlocaledges * sizeof(int64_t));*/\
  g->column = (int32_t*)xrealloc(graph_so_far.column, (size_t)g->nlocaledges * sizeof(int32_t)); \
...
/* g->nglobalverts = INT64_C(1) << graph_so_far.lg_nglobalverts;*/\
  g->nglobalverts = INT32_C(1) << graph_so_far.lg_nglobalverts;
...
void convert_graph_to_oned_csc(const tuple_graph* const tg, oned_csc_graph* const g) { \
  ...
  g->max_nlocalverts = (int32_t)(g->nlocalverts);  // g->max_nlocalverts = (int64_t)(g->nlocalverts);  
//  MPI_Allreduce(MPI_IN_PLACE, &g->max_nlocalverts, 1, MPI_INT64_T, MPI_MAX, MPI_COMM_WORLD);
  MPI_Allreduce(MPI_IN_PLACE, &g->max_nlocalverts, 1, MPI_INT32_T, MPI_MAX, MPI_COMM_WORLD);
// int64_t local_queue_summary_size = (g->max_nlocalverts + ULONG_BITS * ULONG_BITS - 1) / ULONG_BITS / ULONG_BITS;
  int32_t local_queue_summary_size = (g->max_nlocalverts + ULONG_BITS * ULONG_BITS - 1) / ULONG_BITS / ULONG_BITS;
//int64_t local_queue_size = local_queue_summary_size * ULONG_BITS;
  int32_t local_queue_size = local_queue_summary_size * ULONG_BITS;
  if (g->lg_local_queue_size != lg_int32_t(local_queue_size)) // if (g->lg_local_queue_size != lg_int64_t(local_queue_size)) 
  {
/*
...
*/
    fprintf(stderr, "%d: lg_local_queue_size mismatch: graph redistribution computed %d, convert_graph_to_oned_csc outer computed %d from %" PRId32 "\n", rank, g->lg_local_queue_size, lg_int32_t(local_queue_size), local_queue_size);
