...
// int64_t local_queue_summary_size = (maxlocalverts + ULONG_BITS * ULONG_BITS - 1) / ULONG_BITS / ULONG_BITS;
  int32_t local_queue_summary_size = (maxlocalverts + ULONG_BITS * ULONG_BITS - 1) / ULONG_BITS / ULONG_BITS;
//int64_t local_queue_size = local_queue_summary_size * ULONG_BITS;
  int32_t local_queue_size = local_queue_summary_size * ULONG_BITS;
// int64_t global_queue_summary_size = MUL_SIZE(local_queue_summary_size);
  int32_t global_queue_summary_size = MUL_SIZE(local_queue_summary_size);
...
void run_bfs(int32_t root, int32_t* pred) {
  ...
  int32_t local_queue_size = INT32_C(1) << lg_local_queue_size;   // int64_t local_queue_size = INT64_C(1) << lg_local_queue_size; 
  int32_t local_queue_summary_size = local_queue_size / ULONG_BITS;// int64_t local_queue_summary_size = local_queue_size / ULONG_BITS;
...
#pragma omp parallel for schedule(static) reduction(|:not_done)
    for (ii_summary = 0; ii_summary < global_queue_summary_size; ++ii_summary) {
      uint32_t val_summary = in_queue_summary[ii_summary]; // uint64_t val_summary = in_queue_summary[ii_summary]; 
...
// if ((val_summary & (UINT64_C(1) << ii_offset)) == 0) continue;
        ...
        uint32_t val = in_queue[ii];  //     uint64_t val = in_queue[ii]; 
        if (val == 0) continue;
        size_t i, i_end = rowstarts[ii + 1];
        for (i = rowstarts[ii]; i < i_end; ++i) {
          int32_t c = column[i];//int64_t c = column[i];
          int32_t v0_local = c / ULONG_BITS;  // int64_t v0_local = c / ULONG_BITS; 
//if ((val & (UINT64_C(1) << (c % ULONG_BITS))) != 0 /* TEST_IN(v1_swizzled) */ && !TAS_VISITED_LOCAL(v0_local))
          if ((val & (UINT32_C(1) << (c % ULONG_BITS))) != 0 /* TEST_IN(v1_swizzled) */ && !TAS_VISITED_LOCAL(v0_local)) {
            assert (pred[v0_local] == -1);
            int32_t v1_swizzled = (int32_t)ii * ULONG_BITS + c % ULONG_BITS;  //  int64_t v1_swizzled = (int64_t)ii * ULONG_BITS + c % ULONG_BITS; 
