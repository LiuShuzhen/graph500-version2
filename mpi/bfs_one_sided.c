...
void run_bfs(int32_t root, int32_t* pred) {
  ...
  const int32_t nglobalverts = g.nglobalverts;//const int64_t nglobalverts = g.nglobalverts;

  /* Set up a second predecessor map so we can read from one and modify the
   * other. */
  int32_t* orig_pred = pred;//int64_t* orig_pred = pred;
  int32_t* pred2 = (int32_t*)xMPI_Alloc_mem(nlocalverts * sizeof(int64_t));// int64_t* pred2 = (int64_t*)xMPI_Alloc_mem(nlocalverts * sizeof(int64_t));
...
//  int64_t queue_nbits = (nlocalverts + elts_per_queue_bit - 1) / elts_per_queue_bit;
  int32_t queue_nbits = (nlocalverts + elts_per_queue_bit - 1) / elts_per_queue_bit;
...
//  int64_t* local_vertices = (int64_t*)xMPI_Alloc_mem(nlocalverts * sizeof(int64_t));
  int32_t* local_vertices = (int32_t*)xMPI_Alloc_mem(nlocalverts * sizeof(int32_t));
  ...
  {size_t i; for (i = 0; i < nlocalverts; ++i) pred[i] = INT32_MAX;}  // {size_t i; for (i = 0; i < nlocalverts; ++i) pred[i] = INT64_MAX;}
...
// MPI_Win_create(pred, nlocalverts * sizeof(int64_t), sizeof(int64_t), MPI_INFO_NULL, MPI_COMM_WORLD, &pred_win);
  MPI_Win_create(pred, nlocalverts * sizeof(int32_t), sizeof(int32_t), MPI_INFO_NULL, MPI_COMM_WORLD, &pred_win);
//  MPI_Win_create(pred2, nlocalverts * sizeof(int64_t), sizeof(int64_t), MPI_INFO_NULL, MPI_COMM_WORLD, &pred2_win);
  MPI_Win_create(pred2, nlocalverts * sizeof(int32_t), sizeof(int32_t), MPI_INFO_NULL, MPI_COMM_WORLD, &pred2_win);
  ...
  while (1) {
    ...
    memcpy(pred2, pred, nlocalverts * sizeof(int32_t));//    memcpy(pred2, pred, nlocalverts * sizeof(int64_t));
//for (i = 0; i < (int64_t)nlocalverts; ++i) 
    for (i = 0; i < (int32_t)nlocalverts; ++i) {
        ...
        for (qelem_idx = 0; qelem_idx < elts_per_queue_bit; ++qelem_idx) {
          ...
          if (pred[v_local] >= 0 && pred[v_local] < nglobalverts) {
            ...
            for (ei = g.rowstarts[v_local]; ei < ei_end; ++ei) {
              int32_t w = g.column[ei];// int64_t w = g.column[ei];
...
//   MPI_Accumulate(&local_vertices[v_local], 1, MPI_INT64_T, VERTEX_OWNER(w), VERTEX_LOCAL(w), 1, MPI_INT64_T, MPI_MIN, pred2_win);
              MPI_Accumulate(&local_vertices[v_local], 1, MPI_INT32_T, VERTEX_OWNER(w), VERTEX_LOCAL(w), 1, MPI_INT32_T, MPI_MIN, pred2_win);
    ...
    {int32_t* temp = pred; pred = pred2; pred2 = temp;}   //  {int64_t* temp = pred; pred = pred2; pred2 = temp;} 
  }
  ...
  if (pred2 != orig_pred) {
    memcpy(orig_pred, pred2, nlocalverts * sizeof(int32_t));//   memcpy(orig_pred, pred2, nlocalverts * sizeof(int64_t));
    MPI_Free_mem(pred2);
  } else {
    MPI_Free_mem(pred);
  }
