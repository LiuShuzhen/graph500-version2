...
static int32_t g_local_queue_size; //static int64_t g_local_queue_size; 
static int32_t g_global_queue_summary_size;  //static int64_t g_global_queue_summary_size; 
static int32_t g_global_queue_size;  //static int64_t g_global_queue_size;
...
static void allocate_memory(void) {
  int32_t maxlocalverts = g.max_nlocalverts; // int64_t maxlocalverts = g.max_nlocalverts;
// int lg_local_queue_size = lg_int64_t((maxlocalverts + ulong_bits_squared - 1) / ulong_bits_squared * ulong_bits_squared);
  int lg_local_queue_size = lg_int32_t((maxlocalverts + ulong_bits_squared - 1) / ulong_bits_squared * ulong_bits_squared);
...
//  int64_t local_queue_summary_size = (INT64_C(1) << lg_local_queue_size) / ulong_bits_squared;
  int32_t local_queue_summary_size = (INT32_C(1) << lg_local_queue_size) / ulong_bits_squared;
  int32_t local_queue_size = local_queue_summary_size * ulong_bits;// int64_t local_queue_size = local_queue_summary_size * ulong_bits;
  g_local_queue_summary_size = local_queue_summary_size;
  g_local_queue_size = local_queue_size;
  int32_t global_queue_summary_size = MUL_SIZE(local_queue_summary_size);  //  int64_t global_queue_summary_size = MUL_SIZE(local_queue_summary_size); 
  int32_t global_queue_size = MUL_SIZE(local_queue_size);  // int64_t global_queue_size = MUL_SIZE(local_queue_size);
...
void run_bfs(int32_t root, int32_t* pred) {
  ...
  const int32_t* const restrict column = g.column;   //  const int64_t* const restrict column = g.column; 

  /* Set up the visited bitmap. */
  const int ulong_bits = sizeof(unsigned long) * CHAR_BIT;
  const int ulong_bits_squared = ulong_bits * ulong_bits;
  int32_t local_queue_summary_size = g_local_queue_summary_size;//  int64_t local_queue_summary_size = g_local_queue_summary_size;
  int32_t local_queue_size = g_local_queue_size;//  int64_t local_queue_size = g_local_queue_size;
  int lg_local_queue_size = g_lg_local_queue_size;
  int32_t global_queue_summary_size = g_global_queue_summary_size;  //  int64_t global_queue_summary_size = g_global_queue_summary_size;  
  int32_t global_queue_size = g_global_queue_size;  //  int64_t global_queue_size = g_global_queue_size; 
//#define SWIZZLE_VERTEX(c) (((int64_t)(VERTEX_OWNER(c)) << lg_local_queue_size) | (int64_t)(VERTEX_LOCAL(c)))
#define SWIZZLE_VERTEX(c) (((int32_t)(VERTEX_OWNER(c)) << lg_local_queue_size) | (int32_t)(VERTEX_LOCAL(c)))
#if 0
// int64_t* restrict column_swizzled = (int64_t*)xmalloc(nlocaledges * sizeof(int64_t));
  int32_t* restrict column_swizzled = (int32_t*)xmalloc(nlocaledges * sizeof(int32_t));
  {
    size_t i;
    for (i = 0; i < nlocaledges; ++i) {
      int32_t c = column[i];  //   int64_t c = column[i]; 
...
//#define SET_IN(v) do {int64_t vs = SWIZZLE_VERTEX(v); size_t word_idx = vs / ulong_bits; int bit_idx = vs % ulong_bits; unsigned long mask = (1UL << bit_idx);
#define SET_IN(v) do {int32_t vs = SWIZZLE_VERTEX(v); size_t word_idx = vs / ulong_bits; int bit_idx = vs % ulong_bits; unsigned long mask = (1UL << bit_idx); in_queue_summary[word_idx / ulong_bits] |= (1UL << (word_idx % ulong_bits)); in_queue[word_idx] |= mask;} while (0)
      ...
      for (i = ii; i < i_end; ++i) {
        if (!TEST_VISITED_LOCAL(i)) {
          ...
          for (j = rowstarts[i]; j < j_end; ++j) {
            int32_t v1 = column[j];  //  int64_t v1 = column[j]; 
               int32_t v1_swizzled = SWIZZLE_VERTEX(v1);  // int64_t v1_swizzled = SWIZZLE_VERTEX(v1);
            if (TEST_IN(v1_swizzled)) {
              pred[i] = (v1 & INT32_C(0xFFFFFF)) | ((int32_t)cur_level << 24);  // pred[i] = (v1 & INT64_C(0xFFFFFFFFFFFF)) | ((int64_t)cur_level << 48);
              not_done |= 1;
              SET_VISITED_LOCAL(i);
              break;
            }
...
#pragma omp parallel for
  for (i = 0; i < (ptrdiff_t)count; ++i) {
    int32_t v = vertex[i];//    int64_t v = vertex[i];
