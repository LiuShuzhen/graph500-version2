...
static int32_t nvtx_scale;//static int64_t nvtx_scale;(line32)

static struct packed_edge * restrict IJ;
static int32_t nedge;  //static int64_t nedge;(line35)

static int32_t bfs_root[NBFS_max];  //static int64_t bfs_root[NBFS_max]; (line37) 
...
main (int argc, char **argv)
{
  ...
  int fd;
  int32_t desired_nedge;   //int64_t desired_nedge;  (line44)
  if (sizeof (int32_t) < 8) //if (sizeof (int64_t) < 8)  (line45)
  {
  ...
  if (rootname >= 0) {
    ...
    OMP("omp parallel") {
      OMP("omp for")
	for (int32_t k = 0; k < nvtx_scale; ++k)  //for (int64_t k = 0; k < nvtx_scale; ++k) (line98)
	  has_adj[k] = 0;
      MTA("mta assert nodep") OMP("omp for")
	for (int32_t k = 0; k < nedge; ++k) //for (int64_t k = 0; k < nedge; ++k) (line101)
	{
	  const int32_t i = get_v0_from_edge(&IJ[k]);  //  const int64_t i = get_v0_from_edge(&IJ[k]);  (line103) 
	  const int32_t j = get_v1_from_edge(&IJ[k]);  //  const int64_t j = get_v1_from_edge(&IJ[k]);  (line104)
	  if (i != j)
